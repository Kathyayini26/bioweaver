...
print("=" * 60)
print("ZERO-LEAKAGE INDUCTIVE EVALUATION PIPELINE")
print("=" * 60)
...
# Split positive pairs 80/20 BEFORE Node2Vec
train_pos, test_pos = train_test_split(positive_edges, test_size=0.2, random_state=42)

# --------------------------------------------------
# 3. Prune Graph: Remove Test Edges BEFORE Node2Vec!
# --------------------------------------------------
G_train = G.copy()
removed_count = 0
for u, v in test_pos:
...
# --------------------------------------------------
# 4. Fast Node2Vec Embeddings ONLY on G_train
# --------------------------------------------------
print("\nGenerating Node2Vec embeddings on Pruned Training Graph (Zero Leakage)...")
node2vec = Node2Vec(
    G_train,
    dimensions=128,
    walk_length=15,
...
embeddings = {str(node): n2v_model.wv[str(node)] for node in G_train.nodes() if str(node) in n2v_model.wv}
...
print(f"Found {len(genes)} Gene nodes and {len(diseases)} Disease nodes in graph.")
